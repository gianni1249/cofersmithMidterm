...
int main(int argc, char* argv[]){
   if(argc>3){
	cout << "Usage is makeLED and one of: " << endl;
        cout << "   on, off, flash, blink n, or status" << endl;
   ...
   if(cmd=="on"){
	...
	std::fstream fs2;
        fs2.open( LED3_PATH "/brightness", std::fstream::in);
        string line;
        while(getline(fs2,line)){
	if(line=="1"){
	cout << "Toggling the LED off " << endl;
        removeTrigger();
        writeLED("/brightness", "0");
         }   
        if(line=="0"){
        cout << " Toggling the LED on " << endl;
        removeTrigger();
        writeLED("/brightness", "1"); 
    }
}
